...
**玩法演示**（本人实际操作录制：开始界面 → 关卡选择 → 逐关通过第 1~5 关 → 全部通关结算；第 1 关出现一次碰撞失误并使用了撤销）：
...
| 碰撞反馈 | 通关（零失误） | 通关（有失误） | 失败 |
|---|---|---|---|
| ![碰撞反馈](docs/screenshots/02_collision.png) | ![通关](docs/screenshots/03_win.png) | ![通关-有失误](docs/screenshots/09_win_rated.png) | ![失败](docs/screenshots/04_lose.png) |

| 第 5 关（最难关卡） | 全部通关（累计总分与星数） |
|---|---|
| ![第 5 关](docs/screenshots/05_level5.png) | ![全部通关](docs/screenshots/06_all_clear.png) |

以上截图均由脚本驱动真实游戏代码渲染生成，非手工绘制（`tools/capture_screenshots.py`，改动界面后可一键重出）；演示 GIF 为本人实际操作录制。
...
6. 每关从进入关卡起计时，胜负判定时立即停表；结算界面给出本关星级与得分。

### 星级与得分

| 项目 | 规则 |
|---|---|
| 计时 | 从进入关卡开始，到胜负判定为止；结算界面、开始界面与选关界面都不计时 |
| 星级 | 由 **失误次数 + 提示次数** 的累计值决定：0 → ★★★，1 → ★★，2 及以上 → ★（通关至少 1 星，失败不评星） |
| 得分 | 每关满分 1000 分；每次失误 −200 分；超出参考时长后每秒 −10 分；保底 200 分 |
| 参考时长 | 本关箭头数 × 5 秒（第 1 关 8 个箭头 → 40 秒，第 5 关 14 个箭头 → 70 秒） |

**撤销不在评级惩罚之内**：它退还的是失误计数，属于"走错了退回来"的容错机制；
而提示是主动向游戏要答案，因此计入惩罚。重玩同一关时只保留更好的成绩（先比星级，再比得分）。

各关成绩会显示在关卡选择界面的卡片上（星级 + 得分），开始界面则显示累计星数与总分。
...
- **关卡选择**：可直接进入任意一关，已通关的卡片显示该关星级与得分
- **重玩本关**：通关后结算界面提供「重玩本关」，无需退回主菜单重新选关
- **计时**：每关独立计时并在 HUD 实时显示，胜负判定即停表
- **星级评价**：按「失误次数 + 提示次数」评 1 ~ 3 星，结算界面用三颗星展示
- **得分与累计成绩**：每关满分 1000 分（失误与超时扣分、保底 200 分）；开始界面与「全部通关」界面显示累计总分与星数
...
界面顶部的信息栏实时显示：当前关卡名、剩余箭头数、本关用时、剩余失误次数。
...
| 附加功能 | T13 ~ T17（撤销、提示、撤销禁用态、快捷键 U / H）；T22 ~ T24（计时、星级评价、得分） |
| 工程健壮性 | T18 窗口自适应缩放、T19 四个界面渲染冒烟、T20 音效合成与无声卡降级、T21 胜负判定后输入锁定、T25 判定后键盘 U / H 失效 |
