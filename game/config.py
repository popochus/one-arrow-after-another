...
# ---------------- 配色（星级与得分）----------------
COLOR_STAR         = (245, 176, 40)   # 点亮的星（金）
COLOR_STAR_EMPTY   = (222, 227, 236)  # 未点亮的星（灰）
COLOR_STAR_EDGE    = (222, 152, 26)   # 金星描边，浅底上不至于糊成一片
COLOR_SCORE_VALUE  = (56, 108, 222)   # 得分数字
...
# ---------------- 计时与评价 ----------------
# 星级由「失误次数 + 提示次数」的累计惩罚决定：
#   0 点 → 3 星，1 点 → 2 星，2 点及以上 → 1 星。
# 通关就至少 1 星，故星级下限是 STAR_MIN 而非 0（失败一局不评星）。
STAR_MAX = 3
STAR_MIN = 1
STAR_LOSS_PER_PENALTY = 1   # 每 1 点惩罚（一次失误或一次提示）降几星

# 单关得分 = 基础分 - 失误扣分 - 超时扣分，但不会低于保底分。
# 参考时长按「本关箭头数 × SECONDS_PER_ARROW」估算——箭头越多给的时间越长，
# 否则第 5 关（14 个箭头）会比第 1 关（8 个）更容易被判超时。
SCORE_BASE = 1000           # 每关基础分（零失误且不超时即满分）
SCORE_PER_MISTAKE = 200     # 每次失误扣分
SCORE_PER_SECOND = 10       # 超出参考时长后，每秒扣分
SCORE_MIN = 200             # 保底分：只要通关至少得这么多
SECONDS_PER_ARROW = 5.0     # 每支箭头的参考耗时（秒）
